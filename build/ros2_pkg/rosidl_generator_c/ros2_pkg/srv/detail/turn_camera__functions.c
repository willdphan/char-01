  ...
  ros2_pkg__srv__TurnCamera_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ros2_pkg__srv__TurnCamera_Request * data =
      (ros2_pkg__srv__TurnCamera_Request *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros2_pkg__srv__TurnCamera_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ros2_pkg__srv__TurnCamera_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
  ...
  ros2_pkg__srv__TurnCamera_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ros2_pkg__srv__TurnCamera_Response * data =
      (ros2_pkg__srv__TurnCamera_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    ...
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros2_pkg__srv__TurnCamera_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ros2_pkg__srv__TurnCamera_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
